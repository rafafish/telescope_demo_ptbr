...
import 'package:telescope_dml_eleicoes_vinhedo/Screens/5.dart';
...
class EscolaridadeScreen extends StatefulWidget {
  ...
  EscolaridadeScreen() {
    checkItems.add(CheckBoxItems(
        label: 'Até 1.º grau', id: 1, checked: false, enabled: true));
    checkItems.add(CheckBoxItems(
        label: 'Até 2.º grau', id: 2, checked: false, enabled: true));
    checkItems.add(CheckBoxItems(
        label: 'Até Ensino Médio', id: 3, checked: false, enabled: true));
    checkItems.add(CheckBoxItems(
        label: 'Superior incompleto', id: 4, checked: false, enabled: true));
    checkItems.add(CheckBoxItems(
        label: 'Superior Completo', id: 5, checked: false, enabled: true));
    checkItems.add(CheckBoxItems(
        label: 'Pós-Graduado / Mestrado / Doutorado',
        id: 6,
        checked: false,
        enabled: true));
    checkItems.add(CheckBoxItems(
        label: 'Não Respondeu', id: 7, checked: false, enabled: true));
  }

  @override
  _EscolaridadeScreenState createState() => _EscolaridadeScreenState();
}
...
class _EscolaridadeScreenState extends State<EscolaridadeScreen> {
  saveDataSurvey() {
    widget.checkItems.forEach((value) {
      if (value.checked) {
        globals.singleSurvey.escolaridade = value.label;
      }
    });
  }
