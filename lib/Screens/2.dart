...
import 'package:telescope_dml_eleicoes_vinhedo/Screens/3.dart';
...
class SexoScreen extends StatefulWidget {
  ...
  SexoScreen() {
    checkItems.add(CheckBoxItems(
        label: 'MASCULINO', id: 1, checked: false, enabled: true));
    checkItems.add(
        CheckBoxItems(label: 'FEMININO', id: 2, checked: false, enabled: true));
  }

  @override
  _SexoScreenState createState() => _SexoScreenState();
}
...
class _SexoScreenState extends State<SexoScreen> {
  saveDataSurvey() {
    widget.checkItems.forEach((value) {
      if (value.checked) {
        globals.singleSurvey.sexo = value.label;
      }
    });
  }
